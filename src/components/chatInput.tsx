...
export const ConversationChatInput = ({
  ...
  const queryClient = useQueryClient()
  const { mutateAsync } = useMutation({
    mutationKey: ['messages', friendUserId],
    mutationFn: async (content: string) => {
      try {
        const response = await axios.post<GroupMessage>('/api/privateMessage', {
          content,
          friendUserId,
          type: MessageType.TEXT,
          conversationId,
    ...
    onMutate: (content: string) => {
      queryClient.cancelQueries({ queryKey: ['messages', friendUserId] })
      queryClient.setQueryData(['messages', friendUserId], (oldData: any) => {
        if (!oldData || !oldData.pages || oldData.pages.length === 0) {
          return {
            pages: [
              {
